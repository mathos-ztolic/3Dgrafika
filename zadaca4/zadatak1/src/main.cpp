...
int main()
{
    ...
    // ------------------------------------
    Shader ourShader("resources/shaders/container.vert", "resources/shaders/container.frag");
    ...
    while (!glfwWindowShouldClose(window))
    {
	    ...
	    ourShader.setVec3("light.position", camera_position);
...
void processInput(GLFWwindow *window)
{
    ...
    if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
        camera_position -= glm::normalize(glm::cross(camera_direction, camera_up)) * speed;
...
// -------------------------------------------------------
void mouse_callback(GLFWwindow* window, double xd, double yd)
{
    float x = (float) xd;
    float y = (float) yd;
    ...
    // pitch goes from -90 to 90 (down to up), otherwise the screen is flipped
    if (pitch > 89.0f)
        pitch = 89.0f;
    if (pitch < -89.0f)
        pitch = -89.0f;
